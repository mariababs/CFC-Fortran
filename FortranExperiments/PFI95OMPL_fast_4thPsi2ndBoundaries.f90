...
!***************************************************
	...
	call PsiCalc(Nx,My,Kappa2,KappaA,dxx,dyy,Psi,Omega,kPsi,DM2,Tol)
...
!*******************************************************************
	subroutine PsiCalc(Nx,My,Kappa2,KappaA,dxx,dyy,Psi,Omega,kPsi,DM2,Tol)
...
!$OMP DO
	...
	if (i.lt.3.or.i.gt.(Nx+1).or.j.lt.3.or.j.gt.(My+1)) then
	Psi(i,j) = KappaA*(dxx*Omega(i,j)*DM2(i,j) + Psi0(i+1,j) + &
	Psi0(i-1,j) + Kappa2*(Psi0(i,j+1) + Psi0(i,j-1)))
	! print*,"2nd",Psi0(i,j),Psi(i,j)
	else
	! Extra () on each portion of the equation isolate any negative (-) signs that could follow a plus (+) sign.
	! This is done even where not needed for consistency.
	Psi(i,j) = 1.0/(2.0*(dxx+dyy)) * ( &
	! Locations two steps away
	! Locations one step away
	((dyy*Psi0(i-1,j) + dyy*Psi0(i+1,j) + dxx*Psi0(i,j-1) + dxx*Psi0(i,j+1))) + &
	! Remaining Omega term
	(Omega(i,j)*dxx*dyy)*DM2(i,j) )
	! print*,"4th",Psi0(i,j),Psi(i,j)
	endif
