...
!***************************************************
	...
	call OmegaCalc(Nx,My,Cx2,Cy2,alpha,alphaX,alphaY,Omega,Omega0,u,v,DM,DM2,Re,dt,dx,dy,dxx,dyy)
...
!
!*******************************************************************
	subroutine OmegaCalc(Nx,My,Cx2,Cy2,alpha,alphaX,alphaY,Omega,Omega0,u,v,DM,DM2,Re,dt,dx,dy,dxx,dyy)
	integer Nx,My,i,j
	...
	! Handle the bulk
		do 80 i = 3,Nx
			do 90 j = 3,My
		Omega(i,j) = Omega0(i,j) - dt/DM2(i,j)*( &
			8*(DM(i+1,j)*u(i+1,j)*Omega0(i+1,j) - DM(i-1,j)*u(i-1,j)*Omega0(i-1,j))/12.0/dx +&
			-(DM(i+2,j)*u(i+2,j)*Omega0(i+2,j) - DM(i-2,j)*u(i-2,j)*Omega0(i-2,j))/12.0/dx +&
			8*(DM(i,j+1)*v(i,j+1)*Omega0(i,j+1) - DM(i,j-1)*v(i,j-1)*Omega0(i,j-1))/12.0/dy +&
			-(DM(i,j+2)*v(i,j+2)*Omega0(i,j+2) - DM(i,j-2)*v(i,j-2)*Omega0(i,j-2))/12.0/dy &
			) + dt/Re/DM2(i,j)*( &
			(-Omega0(i-2,j) + 16*Omega0(i-1,j) - 30*Omega0(i,j) + 16*Omega0(i+1,j) - Omega0(i+2,j))/12.0/dxx + &
			(-Omega0(i,j-2) + 16*Omega0(i,j-1) - 30*Omega0(i,j) + 16*Omega0(i,j+1) - Omega0(i,j+2))/12.0/dyy &
			)
	90		continue
	80	continue
	!$OMP END DO
	...
	!$OMP DO
		do j = 3,My
		Omega(i,j) = Omega0(i,j) - dt/DM2(i,j)*( &
			(-3*DM(i-1,j)*u(i-1,j)*Omega0(i-1,j) + -10*DM(i,j)*u(i,j)*Omega0(i,j))/12.0/dx +&
			(18*DM(i+1,j)*u(i+1,j)*Omega0(i+1,j) - 6*DM(i+2,j)*u(i+2,j)*Omega0(i+2,j))/12.0/dx +&
			(DM(i+3,j)*u(i+3,j)*Omega0(i+3,j))/12.0/dx +&
			8*(DM(i,j+1)*v(i,j+1)*Omega0(i,j+1) - DM(i,j-1)*v(i,j-1)*Omega0(i,j-1))/12.0/dy +&
			-(DM(i,j+2)*v(i,j+2)*Omega0(i,j+2) - DM(i,j-2)*v(i,j-2)*Omega0(i,j-2))/12.0/dy &
			) + dt/Re/DM2(i,j)*( &
			(11*Omega0(i-1,j) - 20*Omega0(i,j) + 6*Omega0(i+1,j) + 4*Omega0(i+2,j) - Omega0(i+3,j))/12.0/dxx + &
			(-Omega0(i,j) + 16*Omega0(i,j) - 30*Omega0(i,j) + 16*Omega0(i,j) - Omega0(i,j))/12.0/dyy &
			)
		end do
	!$OMP END DO
	...
	!$OMP DO
		do j = 3,My
		Omega(i,j) = Omega0(i,j) - dt/DM2(i,j)*( &
			(-DM(i-3,j)*u(i-3,j)*Omega0(i-3,j) + 6*DM(i-2,j)*u(i-2,j)*Omega0(i-2,j))/12.0/dx +&
			(-18*DM(i-1,j)*u(i-1,j)*Omega0(i-1,j) + 10*DM(i,j)*u(i,j)*Omega0(i,j))/12.0/dx +&
			(3*DM(i+1,j)*u(i+1,j)*Omega0(i+1,j))/12.0/dx +&
			(DM(i,j+1)*v(i,j+1)*Omega0(i,j+1) - DM(i,j-1)*v(i,j-1)*Omega0(i,j-1))/12.0/dy +&
			-(DM(i,j+2)*v(i,j+2)*Omega0(i,j+2) - DM(i,j-2)*v(i,j-2)*Omega0(i,j-2))/12.0/dy &
			) + dt/Re/DM2(i,j)*( &
			(11*Omega0(i+1,j) - 20*Omega0(i,j) + 6*Omega0(i-1,j) + 4*Omega0(i-2,j) - Omega0(i-3,j))/12.0/dxx + &
			(-Omega0(i,j) + 16*Omega0(i,j) - 30*Omega0(i,j) + 16*Omega0(i,j) - Omega0(i,j))/12.0/dyy &
	...
	!$OMP DO
		...
		Omega(i,j) = Omega0(i,j) - dt/DM2(i,j)*( &
			8*(DM(i+1,j)*u(i+1,j)*Omega0(i+1,j) - DM(i-1,j)*u(i-1,j)*Omega0(i-1,j))/12.0/dx +&
			-(DM(i+2,j)*u(i+2,j)*Omega0(i+2,j) - DM(i-2,j)*u(i-2,j)*Omega0(i-2,j))/12.0/dx +&
			(-3*DM(i,j-1)*v(i,j-1)*Omega0(i,j-1) - 10*DM(i,j)*v(i,j)*Omega0(i,j))/12.0/dy +&
			(18*DM(i,j+1)*v(i,j+1)*Omega0(i,j+1) - 6*DM(i,j+2)*v(i,j+2)*Omega0(i,j+2))/12.0/dy +&
			(DM(i,j+3)*v(i,j+3)*Omega0(i,j+3))/12.0/dy &
			) + dt/Re/DM2(i,j)*( &
			(-Omega0(i-2,j) + 16*Omega0(i-1,j) - 30*Omega0(i,j) + 16*Omega0(i+1,j) - Omega0(i+2,j))/12.0/dxx + &
			(11*Omega0(i,j-1) + -20*Omega0(i,j) + 6*Omega0(i,j+1) + 4*Omega0(i,j+2) - Omega0(i,j+3))/12.0/dyy &
			)
		end do
	!OMP END DO
