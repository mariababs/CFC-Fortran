...
!$OMP DO
	...
	if ((i.gt.2).and.(i.lt.(Nx+1))) then
		Omega(i,j) = Omega(i,j) + -Cx/DM2(i,j) * (Omega0(i-2,j) - 8*Omega0(i-1,j) + 8*Omega0(i+1,j) - Omega0(i+2,j)) / 12.0
	else if (i.eq.2) then
		Omega(i,j) = Omega(i,j) -Cx/DM2(i,j) * (-1.0/4*Omega0(i-1,j) - 5.0/6*Omega0(i,j) + 3.0/2*Omega0(i+1,j) - &
		1.0/2*Omega0(i+2,j) + 1.0/12*Omega0(i+3,j)) / 12
	...
	if ((j.gt.2).and.(j.lt.(My+1))) then
		Omega(i,j) = Omega(i,j) -Cy/DM2(i,j) * (Omega0(i,j-2) - 8*Omega0(i,j-1) + 8*Omega0(i,j+1) - Omega0(i,j+2)) / 12.0
	else if (j.eq.2) then
		Omega(i,j) = Omega(i,j) -Cx/DM2(i,j) * (-1.0/4*Omega0(i,j-1) - 5.0/6*Omega0(i,j) + 3.0/2*Omega0(i,j+1) - &
		1.0/2*Omega0(i,j+2) + 1.0/12*Omega0(i,j+3)) / 12
