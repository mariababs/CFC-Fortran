...
!$OMP DO
	...
	! Psi(i,j) = 1.0/2.0*(dxx*dyy*Omega(i,j) + &
	! dyy/(dxx+dyy)*(Psi0(i-1,j)+Psi0(i+1,j)) + &
	! dxx/(dxx+dyy)*(Psi0(i,j-1)+Psi0(i,j+1)))

	if (kPsi.gt.10.and.(i.gt.2).and.(i.lt.(Nx+1))) then
		Psi(i,j) = 2.0/5.0/(dxx+dyy)*(Omega(i,j)*(dxx*dyy) + &
		4.0/3.0*dyy*(Psi0(i-1,j)+Psi0(i+1,j)) + &
		4.0/3.0*dxx*(Psi0(i,j-1)+Psi0(i,j+1)) + &
		-1.0/12.0*dyy*(Psi0(i-2,j)+Psi0(i+2,j)) + &
		-1.0/12.0*dxx*(Psi0(i,j-2)+Psi0(i,j+2)) )
