...
!******************************************************************************
	...
	currentminPsi, minPsi, currentmaxPsi, maxPsi, &
    currentminOmega, minOmega, currentmaxOmega, maxOmega
...
1015		print *,'Would you like to change any of the simulation'
		...
		read *,IBL
		print *,'Input a new Tolerance value equal orless than ',OutDP(7)
		read *,Tol
...
!$OMP DO
	...
	do 110 j = 2,My+1
		...
		if (i == Nx .And. j == My) then
			...
			currentminOmega = minval(Omega)
			currentmaxOmega = maxval(Omega)
			currentminPsi = minval(Psi)
			currentmaxPsi = maxval(Psi)
			minOmega = min(minOmega, currentminOmega)
			maxOmega = max(maxOmega, currentmaxOmega)
			minPsi = min(minPsi, currentminPsi)
			maxPsi = max(maxPsi, currentmaxPsi)
		endif
110	continue
100	continue
!$OMP END DO
